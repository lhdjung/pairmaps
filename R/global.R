...
#   arg_defaults <- rlang::quo_get_expr(rlang::enquo(arg_defaults))
#   # arg_defaults_chr <- rlang::as_label(rlang::quo_get_expr(arg_defaults))
#
#   if (rlang::expr_text(arg_defaults) == "list(.diagonal = NA, .quiet = FALSE)") {
#     return(NULL)
#   } else {
#     rlang::inform((c("i" = "Went further on!")))
#   }
#
#   # return(arg_defaults)
#
#   # # Leave `arg_defaults` as assigned at the top. After checking its first
#   # # element for being `"list"` (right below), only the names should be
#   # # necessary.
#   # if (as.character(arg_defaults)[[1]] != "list") {
#   #   rlang::abort(c(
#   #     "`arg_defaults` must be a (named) list."
#   #   ))
#   # }
#
#   # return(arg_defaults)
#
#   arg_names <- names(arg_defaults)[-1]
#
#   return(arg_names)
#
#   arg_defaults <- rlang::expr_text(arg_defaults)
#
#   arg_names_are_missing <- arg_names[arg_names == ""]
