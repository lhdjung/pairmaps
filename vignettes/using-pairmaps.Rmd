...
The main function of the package is `as_colpair_mapper()`, which creates wrapper functions around `corrr::colpair_map()`. Each wrapper is bound to one specific other function which it applies to a data frame. For example:
...
Use `as_colpair_mapper()` if you plan to reuse the resulting function, like `max_sum_map()`, multiple times. A new wrapper function would add little value if you only want to use it once. In this case, directly calling `corrr::colpair_map()` might be the better option.

## More with corrr

Running your data through `colpair_map()` --- directly or via a wrapper function made by `as_colpair_mapper()` --- will always return a `cor_df` tibble. `cor_df` is the same format as a data frame of correlations returned by `corrr::correlate()`, but it's not limited to computing correlations. Instead, `colpair_map()` arranges its results in this special format after applying any user-specified function which has at least two arguments.
...
The defaults *for these defaults* within `as_colpair_mapper()`, in turn, are equivalent to this:
...
`as_colpair_mapper()` has one more argument, the Boolean `eval_f`. If you set it to `FALSE`, the input function will only appear within the output function by name. Look at `.f = max_sum` here:

```{r}
as_colpair_mapper(f = max_sum, eval_f = FALSE)
```

This is useful if you want to copy and paste the printed output into a script, especially within a package. (See below if you don't work inside of a package.) The default, `eval_f = TRUE`, would replace the name of `max_sum()` by its function definition --- i.e., its list of arguments along with the entire function body:
...
What happens here? The `f = max_sum` argument is only evaluated when the output function, `max_sum_map()`, is eventually called. Before it is, however, the value of `max_sum()` changes to a different function, which is then applied in place of the original `max_sum()`.
...
If the new value can't be interpreted as one or more functions, the bug doesn't occur but an error is thrown:
...
If all you need is the name of the mapped function, call `get_mapped_function_name()` instead:
